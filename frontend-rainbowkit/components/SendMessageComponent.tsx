...
export default function SendMessageComponent(props: Props) {
  const [supportedChains, setSupportedChains] = useState(props.supportedChains);
  const [activeChain, setActiveChain] = useState(props.defaultChain);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      if (!address && openConnectModal) {
        openConnectModal();
      } else if (chain?.id !== activeChain.from.id && switchNetwork) {
  ...
  }, [address, chain?.id, activeChain.from.id]);
...
function NetworkSelector({
  ...
  useEffect(() => {
    const listener = (e: MouseEvent) => {
      if (e.target && selectorRef.current && !selectorRef.current.contains(e.target as Node)) {
        setOpen(false);
      }
    };
